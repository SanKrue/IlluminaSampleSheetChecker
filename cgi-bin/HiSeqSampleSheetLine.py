...
class HiSeqSampleSheetLine:
	# the objects in class HiSeqSampleSheetLine have to have 10 parameters, which were store in SampleSheet files
	...
	def ReadandProcessHiSeq(self, FCID): # parameters are the file (here represented as "self") and the FCID from the filename, 
		...
		for item in fwc:  # for each line in file
			...
			if(len(line) > 0):  # length of the line should be >0
				...
				sampleSheetLineElement = HiSeqSampleSheetLine(splitLine[0], splitLine[1], splitLine[2], splitLine[3], \
													 splitLine[4], splitLine[5], splitLine[6], splitLine[7], \
													 splitLine[8], splitLine[9])# getting all items as HiSeqSampleSheetLine
